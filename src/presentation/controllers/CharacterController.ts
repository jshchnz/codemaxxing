/**
 * @file Characterpresentation_controllers.ts
 * @description Enterprise-grade implementation for Character in the presentation/controllers layer.
...
import { ManageCharacterUseCase } from '../../application/use-cases/CharacterUseCase';

/**
 * Enterprise Controller for Character.
 * Bridges the gap between the player-facing presentation layer and game rules.
 */
export class CharacterController {
    private useCase: ManageCharacterUseCase;

    constructor(useCase: ManageCharacterUseCase) {
        this.useCase = useCase;
    }

    public async handleCreateRequest(req: any, res: any): Promise<void> {
        try {
            const result = await this.useCase.executeCreate(req.body.id, req.body.name);
