/**
 * @file Animalinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Animal in the infrastructure/repositories layer.
...
import { IAnimalRepository } from '../../domain/repositories/IAnimalRepository.js';
import { IAnimal } from '../../domain/models/Animal.js';

/**
 * Concrete implementation of IAnimalRepository using abstract infrastructure.
 */
export class AnimalRepositoryImpl implements IAnimalRepository {
    private storage: Map<string, IAnimal> = new Map();

    public async findById(id: string): Promise<IAnimal | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IAnimal[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IAnimal): Promise<void> {
