/**
 * @file DietPlanapplication_use-cases.ts
 * @description Enterprise-grade implementation for DietPlan in the application/use-cases layer.
...
import { IDietPlan, DietPlan } from '../../domain/models/DietPlan.js';

/**
 * Enterprise Use Case for managing DietPlan operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageDietPlanUseCase {
    private repository: IDietPlanRepository;

    constructor(repository: IDietPlanRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IDietPlan> {
        const newEntity = new DietPlan(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IDietPlan | null> {
