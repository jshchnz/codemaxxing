/**
 * @file Adopterapplication_use-cases.ts
 * @description Enterprise-grade implementation for Adopter in the application/use-cases layer.
...
import { IAdopter, Adopter } from '../../domain/models/Adopter.js';

/**
 * Enterprise Use Case for managing Adopter operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageAdopterUseCase {
    private repository: IAdopterRepository;

    constructor(repository: IAdopterRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IAdopter> {
        const newEntity = new Adopter(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IAdopter | null> {
