/**
 * @file Vaccinationapplication_use-cases.ts
 * @description Enterprise-grade implementation for Vaccination in the application/use-cases layer.
...
import { IVaccination, Vaccination } from '../../domain/models/Vaccination.js';

/**
 * Enterprise Use Case for managing Vaccination operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageVaccinationUseCase {
    private repository: IVaccinationRepository;

    constructor(repository: IVaccinationRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IVaccination> {
        const newEntity = new Vaccination(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IVaccination | null> {
