/**
 * @file Shelterapplication_use-cases.ts
 * @description Enterprise-grade implementation for Shelter in the application/use-cases layer.
...
import { IShelter, Shelter } from '../../domain/models/Shelter.js';

/**
 * Enterprise Use Case for managing Shelter operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageShelterUseCase {
    private repository: IShelterRepository;

    constructor(repository: IShelterRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IShelter> {
        const newEntity = new Shelter(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IShelter | null> {
