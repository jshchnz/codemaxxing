/**
 * @file Veterinarianapplication_use-cases.ts
 * @description Enterprise-grade implementation for Veterinarian in the application/use-cases layer.
...
import { IVeterinarian, Veterinarian } from '../../domain/models/Veterinarian.js';

/**
 * Enterprise Use Case for managing Veterinarian operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageVeterinarianUseCase {
    private repository: IVeterinarianRepository;

    constructor(repository: IVeterinarianRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IVeterinarian> {
        const newEntity = new Veterinarian(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IVeterinarian | null> {
