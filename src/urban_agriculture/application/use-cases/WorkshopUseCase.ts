/**
 * @file Workshopapplication_use-cases.ts
 * @description Enterprise-grade implementation for Workshop in the application/use-cases layer.
...
import { IWorkshop, Workshop } from '../../domain/models/Workshop';

/**
 * Enterprise Use Case for managing Workshop operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageWorkshopUseCase {
    private repository: IWorkshopRepository;

    constructor(repository: IWorkshopRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IWorkshop> {
        const newEntity = new Workshop(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IWorkshop | null> {
