/**
 * @file PruningTaskapplication_use-cases.ts
 * @description Enterprise-grade implementation for PruningTask in the application/use-cases layer.
...
import { IPruningTask, PruningTask } from '../../domain/models/PruningTask';

/**
 * Enterprise Use Case for managing PruningTask operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManagePruningTaskUseCase {
    private repository: IPruningTaskRepository;

    constructor(repository: IPruningTaskRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IPruningTask> {
        const newEntity = new PruningTask(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IPruningTask | null> {
