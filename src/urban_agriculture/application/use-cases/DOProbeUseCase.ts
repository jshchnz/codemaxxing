/**
 * @file DOProbeapplication_use-cases.ts
 * @description Enterprise-grade implementation for DOProbe in the application/use-cases layer.
...
import { IDOProbe, DOProbe } from '../../domain/models/DOProbe';

/**
 * Enterprise Use Case for managing DOProbe operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageDOProbeUseCase {
    private repository: IDOProbeRepository;

    constructor(repository: IDOProbeRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IDOProbe> {
        const newEntity = new DOProbe(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IDOProbe | null> {
