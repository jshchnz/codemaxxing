/**
 * @file ECMeterapplication_use-cases.ts
 * @description Enterprise-grade implementation for ECMeter in the application/use-cases layer.
...
import { IECMeter, ECMeter } from '../../domain/models/ECMeter';

/**
 * Enterprise Use Case for managing ECMeter operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageECMeterUseCase {
    private repository: IECMeterRepository;

    constructor(repository: IECMeterRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IECMeter> {
        const newEntity = new ECMeter(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IECMeter | null> {
