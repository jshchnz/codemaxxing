/**
 * @file Biocharapplication_use-cases.ts
 * @description Enterprise-grade implementation for Biochar in the application/use-cases layer.
...
import { IBiochar, Biochar } from '../../domain/models/Biochar';

/**
 * Enterprise Use Case for managing Biochar operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageBiocharUseCase {
    private repository: IBiocharRepository;

    constructor(repository: IBiocharRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IBiochar> {
        const newEntity = new Biochar(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IBiochar | null> {
