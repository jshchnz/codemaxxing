/**
 * @file Legumeapplication_use-cases.ts
 * @description Enterprise-grade implementation for Legume in the application/use-cases layer.
...
import { ILegume, Legume } from '../../domain/models/Legume';

/**
 * Enterprise Use Case for managing Legume operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageLegumeUseCase {
    private repository: ILegumeRepository;

    constructor(repository: ILegumeRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<ILegume> {
        const newEntity = new Legume(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ILegume | null> {
