/**
 * @file HarvestLogapplication_use-cases.ts
 * @description Enterprise-grade implementation for HarvestLog in the application/use-cases layer.
...
import { IHarvestLog, HarvestLog } from '../../domain/models/HarvestLog';

/**
 * Enterprise Use Case for managing HarvestLog operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageHarvestLogUseCase {
    private repository: IHarvestLogRepository;

    constructor(repository: IHarvestLogRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IHarvestLog> {
        const newEntity = new HarvestLog(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IHarvestLog | null> {
