/**
 * @file Greenhouseapplication_use-cases.ts
 * @description Enterprise-grade implementation for Greenhouse in the application/use-cases layer.
...
import { IGreenhouse, Greenhouse } from '../../domain/models/Greenhouse';

/**
 * Enterprise Use Case for managing Greenhouse operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageGreenhouseUseCase {
    private repository: IGreenhouseRepository;

    constructor(repository: IGreenhouseRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IGreenhouse> {
        const newEntity = new Greenhouse(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IGreenhouse | null> {
