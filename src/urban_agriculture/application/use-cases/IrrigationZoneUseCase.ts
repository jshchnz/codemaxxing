/**
 * @file IrrigationZoneapplication_use-cases.ts
 * @description Enterprise-grade implementation for IrrigationZone in the application/use-cases layer.
...
import { IIrrigationZone, IrrigationZone } from '../../domain/models/IrrigationZone';

/**
 * Enterprise Use Case for managing IrrigationZone operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageIrrigationZoneUseCase {
    private repository: IIrrigationZoneRepository;

    constructor(repository: IIrrigationZoneRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IIrrigationZone> {
        const newEntity = new IrrigationZone(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IIrrigationZone | null> {
