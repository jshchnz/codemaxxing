/**
 * @file Herbapplication_use-cases.ts
 * @description Enterprise-grade implementation for Herb in the application/use-cases layer.
...
import { IHerb, Herb } from '../../domain/models/Herb';

/**
 * Enterprise Use Case for managing Herb operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageHerbUseCase {
    private repository: IHerbRepository;

    constructor(repository: IHerbRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IHerb> {
        const newEntity = new Herb(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IHerb | null> {
