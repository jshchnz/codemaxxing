/**
 * @file PestObservationapplication_use-cases.ts
 * @description Enterprise-grade implementation for PestObservation in the application/use-cases layer.
...
import { IPestObservation, PestObservation } from '../../domain/models/PestObservation';

/**
 * Enterprise Use Case for managing PestObservation operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManagePestObservationUseCase {
    private repository: IPestObservationRepository;

    constructor(repository: IPestObservationRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IPestObservation> {
        const newEntity = new PestObservation(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IPestObservation | null> {
