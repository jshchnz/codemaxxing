/**
 * @file RootCropapplication_use-cases.ts
 * @description Enterprise-grade implementation for RootCrop in the application/use-cases layer.
...
import { IRootCrop, RootCrop } from '../../domain/models/RootCrop';

/**
 * Enterprise Use Case for managing RootCrop operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageRootCropUseCase {
    private repository: IRootCropRepository;

    constructor(repository: IRootCropRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IRootCrop> {
        const newEntity = new RootCrop(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IRootCrop | null> {
