/**
 * @file PumpScheduleapplication_use-cases.ts
 * @description Enterprise-grade implementation for PumpSchedule in the application/use-cases layer.
...
import { IPumpSchedule, PumpSchedule } from '../../domain/models/PumpSchedule';

/**
 * Enterprise Use Case for managing PumpSchedule operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManagePumpScheduleUseCase {
    private repository: IPumpScheduleRepository;

    constructor(repository: IPumpScheduleRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IPumpSchedule> {
        const newEntity = new PumpSchedule(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IPumpSchedule | null> {
