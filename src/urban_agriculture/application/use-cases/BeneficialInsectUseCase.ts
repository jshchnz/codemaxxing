/**
 * @file BeneficialInsectapplication_use-cases.ts
 * @description Enterprise-grade implementation for BeneficialInsect in the application/use-cases layer.
...
import { IBeneficialInsect, BeneficialInsect } from '../../domain/models/BeneficialInsect';

/**
 * Enterprise Use Case for managing BeneficialInsect operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageBeneficialInsectUseCase {
    private repository: IBeneficialInsectRepository;

    constructor(repository: IBeneficialInsectRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IBeneficialInsect> {
        const newEntity = new BeneficialInsect(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IBeneficialInsect | null> {
