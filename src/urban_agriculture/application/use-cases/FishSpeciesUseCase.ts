/**
 * @file FishSpeciesapplication_use-cases.ts
 * @description Enterprise-grade implementation for FishSpecies in the application/use-cases layer.
...
import { IFishSpecies, FishSpecies } from '../../domain/models/FishSpecies';

/**
 * Enterprise Use Case for managing FishSpecies operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageFishSpeciesUseCase {
    private repository: IFishSpeciesRepository;

    constructor(repository: IFishSpeciesRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IFishSpecies> {
        const newEntity = new FishSpecies(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IFishSpecies | null> {
