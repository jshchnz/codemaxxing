/**
 * @file YieldTrackerapplication_use-cases.ts
 * @description Enterprise-grade implementation for YieldTracker in the application/use-cases layer.
...
import { IYieldTracker, YieldTracker } from '../../domain/models/YieldTracker';

/**
 * Enterprise Use Case for managing YieldTracker operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageYieldTrackerUseCase {
    private repository: IYieldTrackerRepository;

    constructor(repository: IYieldTrackerRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IYieldTracker> {
        const newEntity = new YieldTracker(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IYieldTracker | null> {
