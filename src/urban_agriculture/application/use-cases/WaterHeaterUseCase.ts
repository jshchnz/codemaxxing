/**
 * @file WaterHeaterapplication_use-cases.ts
 * @description Enterprise-grade implementation for WaterHeater in the application/use-cases layer.
...
import { IWaterHeater, WaterHeater } from '../../domain/models/WaterHeater';

/**
 * Enterprise Use Case for managing WaterHeater operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageWaterHeaterUseCase {
    private repository: IWaterHeaterRepository;

    constructor(repository: IWaterHeaterRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IWaterHeater> {
        const newEntity = new WaterHeater(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IWaterHeater | null> {
