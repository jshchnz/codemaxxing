/**
 * @file TemperatureSensorapplication_use-cases.ts
 * @description Enterprise-grade implementation for TemperatureSensor in the application/use-cases layer.
...
import { ITemperatureSensor, TemperatureSensor } from '../../domain/models/TemperatureSensor';

/**
 * Enterprise Use Case for managing TemperatureSensor operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageTemperatureSensorUseCase {
    private repository: ITemperatureSensorRepository;

    constructor(repository: ITemperatureSensorRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<ITemperatureSensor> {
        const newEntity = new TemperatureSensor(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ITemperatureSensor | null> {
