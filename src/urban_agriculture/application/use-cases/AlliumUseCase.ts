/**
 * @file Alliumapplication_use-cases.ts
 * @description Enterprise-grade implementation for Allium in the application/use-cases layer.
...
import { IAllium, Allium } from '../../domain/models/Allium';

/**
 * Enterprise Use Case for managing Allium operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageAlliumUseCase {
    private repository: IAlliumRepository;

    constructor(repository: IAlliumRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IAllium> {
        const newEntity = new Allium(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IAllium | null> {
