/**
 * @file TaskQueueapplication_use-cases.ts
 * @description Enterprise-grade implementation for TaskQueue in the application/use-cases layer.
...
import { ITaskQueue, TaskQueue } from '../../domain/models/TaskQueue';

/**
 * Enterprise Use Case for managing TaskQueue operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageTaskQueueUseCase {
    private repository: ITaskQueueRepository;

    constructor(repository: ITaskQueueRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<ITaskQueue> {
        const newEntity = new TaskQueue(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ITaskQueue | null> {
