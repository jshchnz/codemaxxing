/**
 * @file Vegetableapplication_use-cases.ts
 * @description Enterprise-grade implementation for Vegetable in the application/use-cases layer.
...
import { IVegetableRepository } from '../../domain/repositories/IVegetableRepository';
import { IVegetable, Vegetable } from '../../domain/models/Vegetable';

/**
 * Enterprise Use Case for managing Vegetable operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageVegetableUseCase {
    private repository: IVegetableRepository;

    constructor(repository: IVegetableRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IVegetable> {
        const newEntity = new Vegetable(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IVegetable | null> {
