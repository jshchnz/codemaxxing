/**
 * @file Microgreeninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Microgreen in the infrastructure/repositories layer.
...
import { IMicrogreenRepository } from '../../domain/repositories/IMicrogreenRepository';
import { IMicrogreen } from '../../domain/models/Microgreen';

/**
 * Concrete implementation of IMicrogreenRepository using abstract infrastructure.
 */
export class MicrogreenRepositoryImpl implements IMicrogreenRepository {
    private storage: Map<string, IMicrogreen> = new Map();

    public async findById(id: string): Promise<IMicrogreen | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IMicrogreen[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IMicrogreen): Promise<void> {
