/**
 * @file GreenManureinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for GreenManure in the infrastructure/repositories layer.
...
import { IGreenManure } from '../../domain/models/GreenManure';

/**
 * Concrete implementation of IGreenManureRepository using abstract infrastructure.
 */
export class GreenManureRepositoryImpl implements IGreenManureRepository {
    private storage: Map<string, IGreenManure> = new Map();

    public async findById(id: string): Promise<IGreenManure | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IGreenManure[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IGreenManure): Promise<void> {
