/**
 * @file DOProbeinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for DOProbe in the infrastructure/repositories layer.
...
import { IDOProbeRepository } from '../../domain/repositories/IDOProbeRepository';
import { IDOProbe } from '../../domain/models/DOProbe';

/**
 * Concrete implementation of IDOProbeRepository using abstract infrastructure.
 */
export class DOProbeRepositoryImpl implements IDOProbeRepository {
    private storage: Map<string, IDOProbe> = new Map();

    public async findById(id: string): Promise<IDOProbe | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IDOProbe[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IDOProbe): Promise<void> {
