/**
 * @file SeedBankinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for SeedBank in the infrastructure/repositories layer.
...
import { ISeedBankRepository } from '../../domain/repositories/ISeedBankRepository';
import { ISeedBank } from '../../domain/models/SeedBank';

/**
 * Concrete implementation of ISeedBankRepository using abstract infrastructure.
 */
export class SeedBankRepositoryImpl implements ISeedBankRepository {
    private storage: Map<string, ISeedBank> = new Map();

    public async findById(id: string): Promise<ISeedBank | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISeedBank[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISeedBank): Promise<void> {
