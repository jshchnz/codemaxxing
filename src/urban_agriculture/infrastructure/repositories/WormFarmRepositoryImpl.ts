/**
 * @file WormFarminfrastructure_repositories.ts
 * @description Enterprise-grade implementation for WormFarm in the infrastructure/repositories layer.
...
import { IWormFarmRepository } from '../../domain/repositories/IWormFarmRepository';
import { IWormFarm } from '../../domain/models/WormFarm';

/**
 * Concrete implementation of IWormFarmRepository using abstract infrastructure.
 */
export class WormFarmRepositoryImpl implements IWormFarmRepository {
    private storage: Map<string, IWormFarm> = new Map();

    public async findById(id: string): Promise<IWormFarm | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IWormFarm[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IWormFarm): Promise<void> {
