/**
 * @file RainBarrelinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for RainBarrel in the infrastructure/repositories layer.
...
import { IRainBarrelRepository } from '../../domain/repositories/IRainBarrelRepository';
import { IRainBarrel } from '../../domain/models/RainBarrel';

/**
 * Concrete implementation of IRainBarrelRepository using abstract infrastructure.
 */
export class RainBarrelRepositoryImpl implements IRainBarrelRepository {
    private storage: Map<string, IRainBarrel> = new Map();

    public async findById(id: string): Promise<IRainBarrel | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IRainBarrel[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IRainBarrel): Promise<void> {
