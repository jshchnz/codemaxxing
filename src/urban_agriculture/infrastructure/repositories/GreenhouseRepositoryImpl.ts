/**
 * @file Greenhouseinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Greenhouse in the infrastructure/repositories layer.
...
import { IGreenhouseRepository } from '../../domain/repositories/IGreenhouseRepository';
import { IGreenhouse } from '../../domain/models/Greenhouse';

/**
 * Concrete implementation of IGreenhouseRepository using abstract infrastructure.
 */
export class GreenhouseRepositoryImpl implements IGreenhouseRepository {
    private storage: Map<string, IGreenhouse> = new Map();

    public async findById(id: string): Promise<IGreenhouse | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IGreenhouse[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IGreenhouse): Promise<void> {
