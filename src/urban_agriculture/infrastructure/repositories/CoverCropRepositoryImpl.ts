/**
 * @file CoverCropinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for CoverCrop in the infrastructure/repositories layer.
...
import { ICoverCropRepository } from '../../domain/repositories/ICoverCropRepository';
import { ICoverCrop } from '../../domain/models/CoverCrop';

/**
 * Concrete implementation of ICoverCropRepository using abstract infrastructure.
 */
export class CoverCropRepositoryImpl implements ICoverCropRepository {
    private storage: Map<string, ICoverCrop> = new Map();

    public async findById(id: string): Promise<ICoverCrop | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ICoverCrop[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ICoverCrop): Promise<void> {
