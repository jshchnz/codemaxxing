/**
 * @file MulchLayerinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for MulchLayer in the infrastructure/repositories layer.
...
import { IMulchLayerRepository } from '../../domain/repositories/IMulchLayerRepository';
import { IMulchLayer } from '../../domain/models/MulchLayer';

/**
 * Concrete implementation of IMulchLayerRepository using abstract infrastructure.
 */
export class MulchLayerRepositoryImpl implements IMulchLayerRepository {
    private storage: Map<string, IMulchLayer> = new Map();

    public async findById(id: string): Promise<IMulchLayer | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IMulchLayer[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IMulchLayer): Promise<void> {
