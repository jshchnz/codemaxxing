/**
 * @file TaskQueueinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for TaskQueue in the infrastructure/repositories layer.
...
import { ITaskQueueRepository } from '../../domain/repositories/ITaskQueueRepository';
import { ITaskQueue } from '../../domain/models/TaskQueue';

/**
 * Concrete implementation of ITaskQueueRepository using abstract infrastructure.
 */
export class TaskQueueRepositoryImpl implements ITaskQueueRepository {
    private storage: Map<string, ITaskQueue> = new Map();

    public async findById(id: string): Promise<ITaskQueue | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ITaskQueue[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ITaskQueue): Promise<void> {
