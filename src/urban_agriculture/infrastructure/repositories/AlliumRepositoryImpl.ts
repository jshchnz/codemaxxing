/**
 * @file Alliuminfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Allium in the infrastructure/repositories layer.
...
import { IAlliumRepository } from '../../domain/repositories/IAlliumRepository';
import { IAllium } from '../../domain/models/Allium';

/**
 * Concrete implementation of IAlliumRepository using abstract infrastructure.
 */
export class AlliumRepositoryImpl implements IAlliumRepository {
    private storage: Map<string, IAllium> = new Map();

    public async findById(id: string): Promise<IAllium | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IAllium[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IAllium): Promise<void> {
