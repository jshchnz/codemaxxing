/**
 * @file Cucurbitinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Cucurbit in the infrastructure/repositories layer.
...
import { ICucurbitRepository } from '../../domain/repositories/ICucurbitRepository';
import { ICucurbit } from '../../domain/models/Cucurbit';

/**
 * Concrete implementation of ICucurbitRepository using abstract infrastructure.
 */
export class CucurbitRepositoryImpl implements ICucurbitRepository {
    private storage: Map<string, ICucurbit> = new Map();

    public async findById(id: string): Promise<ICucurbit | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ICucurbit[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ICucurbit): Promise<void> {
