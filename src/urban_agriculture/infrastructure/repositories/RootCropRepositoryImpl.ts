/**
 * @file RootCropinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for RootCrop in the infrastructure/repositories layer.
...
import { IRootCropRepository } from '../../domain/repositories/IRootCropRepository';
import { IRootCrop } from '../../domain/models/RootCrop';

/**
 * Concrete implementation of IRootCropRepository using abstract infrastructure.
 */
export class RootCropRepositoryImpl implements IRootCropRepository {
    private storage: Map<string, IRootCrop> = new Map();

    public async findById(id: string): Promise<IRootCrop | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IRootCrop[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IRootCrop): Promise<void> {
