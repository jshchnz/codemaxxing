/**
 * @file GrowLightinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for GrowLight in the infrastructure/repositories layer.
...
import { IGrowLightRepository } from '../../domain/repositories/IGrowLightRepository';
import { IGrowLight } from '../../domain/models/GrowLight';

/**
 * Concrete implementation of IGrowLightRepository using abstract infrastructure.
 */
export class GrowLightRepositoryImpl implements IGrowLightRepository {
    private storage: Map<string, IGrowLight> = new Map();

    public async findById(id: string): Promise<IGrowLight | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IGrowLight[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IGrowLight): Promise<void> {
