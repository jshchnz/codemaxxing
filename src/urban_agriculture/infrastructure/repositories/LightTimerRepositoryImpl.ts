/**
 * @file LightTimerinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for LightTimer in the infrastructure/repositories layer.
...
import { ILightTimerRepository } from '../../domain/repositories/ILightTimerRepository';
import { ILightTimer } from '../../domain/models/LightTimer';

/**
 * Concrete implementation of ILightTimerRepository using abstract infrastructure.
 */
export class LightTimerRepositoryImpl implements ILightTimerRepository {
    private storage: Map<string, ILightTimer> = new Map();

    public async findById(id: string): Promise<ILightTimer | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ILightTimer[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ILightTimer): Promise<void> {
