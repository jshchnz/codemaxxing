/**
 * @file PestObservationinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for PestObservation in the infrastructure/repositories layer.
...
import { IPestObservationRepository } from '../../domain/repositories/IPestObservationRepository';
import { IPestObservation } from '../../domain/models/PestObservation';

/**
 * Concrete implementation of IPestObservationRepository using abstract infrastructure.
 */
export class PestObservationRepositoryImpl implements IPestObservationRepository {
    private storage: Map<string, IPestObservation> = new Map();

    public async findById(id: string): Promise<IPestObservation | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IPestObservation[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IPestObservation): Promise<void> {
