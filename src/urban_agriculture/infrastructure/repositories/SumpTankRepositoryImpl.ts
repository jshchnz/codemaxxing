/**
 * @file SumpTankinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for SumpTank in the infrastructure/repositories layer.
...
import { ISumpTankRepository } from '../../domain/repositories/ISumpTankRepository';
import { ISumpTank } from '../../domain/models/SumpTank';

/**
 * Concrete implementation of ISumpTankRepository using abstract infrastructure.
 */
export class SumpTankRepositoryImpl implements ISumpTankRepository {
    private storage: Map<string, ISumpTank> = new Map();

    public async findById(id: string): Promise<ISumpTank | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISumpTank[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISumpTank): Promise<void> {
