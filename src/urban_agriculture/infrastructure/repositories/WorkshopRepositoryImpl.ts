/**
 * @file Workshopinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Workshop in the infrastructure/repositories layer.
...
import { IWorkshopRepository } from '../../domain/repositories/IWorkshopRepository';
import { IWorkshop } from '../../domain/models/Workshop';

/**
 * Concrete implementation of IWorkshopRepository using abstract infrastructure.
 */
export class WorkshopRepositoryImpl implements IWorkshopRepository {
    private storage: Map<string, IWorkshop> = new Map();

    public async findById(id: string): Promise<IWorkshop | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IWorkshop[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IWorkshop): Promise<void> {
