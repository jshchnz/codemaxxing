/**
 * @file CSAinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for CSA in the infrastructure/repositories layer.
...
import { ICSARepository } from '../../domain/repositories/ICSARepository';
import { ICSA } from '../../domain/models/CSA';

/**
 * Concrete implementation of ICSARepository using abstract infrastructure.
 */
export class CSARepositoryImpl implements ICSARepository {
    private storage: Map<string, ICSA> = new Map();

    public async findById(id: string): Promise<ICSA | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ICSA[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ICSA): Promise<void> {
