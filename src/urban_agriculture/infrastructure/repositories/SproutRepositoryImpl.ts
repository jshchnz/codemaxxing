/**
 * @file Sproutinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Sprout in the infrastructure/repositories layer.
...
import { ISprout } from '../../domain/models/Sprout';

/**
 * Concrete implementation of ISproutRepository using abstract infrastructure.
 */
export class SproutRepositoryImpl implements ISproutRepository {
    private storage: Map<string, ISprout> = new Map();

    public async findById(id: string): Promise<ISprout | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISprout[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISprout): Promise<void> {
