/**
 * @file RooftopGardeninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for RooftopGarden in the infrastructure/repositories layer.
...
import { IRooftopGardenRepository } from '../../domain/repositories/IRooftopGardenRepository';
import { IRooftopGarden } from '../../domain/models/RooftopGarden';

/**
 * Concrete implementation of IRooftopGardenRepository using abstract infrastructure.
 */
export class RooftopGardenRepositoryImpl implements IRooftopGardenRepository {
    private storage: Map<string, IRooftopGarden> = new Map();

    public async findById(id: string): Promise<IRooftopGarden | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IRooftopGarden[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IRooftopGarden): Promise<void> {
