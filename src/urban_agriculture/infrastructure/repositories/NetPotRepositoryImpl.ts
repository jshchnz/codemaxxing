/**
 * @file NetPotinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for NetPot in the infrastructure/repositories layer.
...
import { INetPotRepository } from '../../domain/repositories/INetPotRepository';
import { INetPot } from '../../domain/models/NetPot';

/**
 * Concrete implementation of INetPotRepository using abstract infrastructure.
 */
export class NetPotRepositoryImpl implements INetPotRepository {
    private storage: Map<string, INetPot> = new Map();

    public async findById(id: string): Promise<INetPot | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<INetPot[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: INetPot): Promise<void> {
