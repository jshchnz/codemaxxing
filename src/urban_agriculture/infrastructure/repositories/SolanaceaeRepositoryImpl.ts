/**
 * @file Solanaceaeinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Solanaceae in the infrastructure/repositories layer.
...
import { ISolanaceaeRepository } from '../../domain/repositories/ISolanaceaeRepository';
import { ISolanaceae } from '../../domain/models/Solanaceae';

/**
 * Concrete implementation of ISolanaceaeRepository using abstract infrastructure.
 */
export class SolanaceaeRepositoryImpl implements ISolanaceaeRepository {
    private storage: Map<string, ISolanaceae> = new Map();

    public async findById(id: string): Promise<ISolanaceae | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISolanaceae[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISolanaceae): Promise<void> {
