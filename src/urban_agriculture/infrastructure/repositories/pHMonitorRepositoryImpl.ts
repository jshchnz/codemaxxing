/**
 * @file pHMonitorinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for pHMonitor in the infrastructure/repositories layer.
...
import { IpHMonitorRepository } from '../../domain/repositories/IpHMonitorRepository';
import { IpHMonitor } from '../../domain/models/pHMonitor';

/**
 * Concrete implementation of IpHMonitorRepository using abstract infrastructure.
 */
export class pHMonitorRepositoryImpl implements IpHMonitorRepository {
    private storage: Map<string, IpHMonitor> = new Map();

    public async findById(id: string): Promise<IpHMonitor | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IpHMonitor[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IpHMonitor): Promise<void> {
