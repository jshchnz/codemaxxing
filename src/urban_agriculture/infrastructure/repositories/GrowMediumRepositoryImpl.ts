/**
 * @file GrowMediuminfrastructure_repositories.ts
 * @description Enterprise-grade implementation for GrowMedium in the infrastructure/repositories layer.
...
import { IGrowMediumRepository } from '../../domain/repositories/IGrowMediumRepository';
import { IGrowMedium } from '../../domain/models/GrowMedium';

/**
 * Concrete implementation of IGrowMediumRepository using abstract infrastructure.
 */
export class GrowMediumRepositoryImpl implements IGrowMediumRepository {
    private storage: Map<string, IGrowMedium> = new Map();

    public async findById(id: string): Promise<IGrowMedium | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IGrowMedium[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IGrowMedium): Promise<void> {
