/**
 * @file Legumeinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Legume in the infrastructure/repositories layer.
...
import { ILegumeRepository } from '../../domain/repositories/ILegumeRepository';
import { ILegume } from '../../domain/models/Legume';

/**
 * Concrete implementation of ILegumeRepository using abstract infrastructure.
 */
export class LegumeRepositoryImpl implements ILegumeRepository {
    private storage: Map<string, ILegume> = new Map();

    public async findById(id: string): Promise<ILegume | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ILegume[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ILegume): Promise<void> {
