/**
 * @file Chillerinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Chiller in the infrastructure/repositories layer.
...
import { IChillerRepository } from '../../domain/repositories/IChillerRepository';
import { IChiller } from '../../domain/models/Chiller';

/**
 * Concrete implementation of IChillerRepository using abstract infrastructure.
 */
export class ChillerRepositoryImpl implements IChillerRepository {
    private storage: Map<string, IChiller> = new Map();

    public async findById(id: string): Promise<IChiller | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IChiller[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IChiller): Promise<void> {
