/**
 * @file Biocharinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Biochar in the infrastructure/repositories layer.
...
import { IBiocharRepository } from '../../domain/repositories/IBiocharRepository';
import { IBiochar } from '../../domain/models/Biochar';

/**
 * Concrete implementation of IBiocharRepository using abstract infrastructure.
 */
export class BiocharRepositoryImpl implements IBiocharRepository {
    private storage: Map<string, IBiochar> = new Map();

    public async findById(id: string): Promise<IBiochar | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IBiochar[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IBiochar): Promise<void> {
