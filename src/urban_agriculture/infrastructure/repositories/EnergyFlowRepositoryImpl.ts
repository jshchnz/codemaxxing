/**
 * @file EnergyFlowinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for EnergyFlow in the infrastructure/repositories layer.
...
import { IEnergyFlowRepository } from '../../domain/repositories/IEnergyFlowRepository';
import { IEnergyFlow } from '../../domain/models/EnergyFlow';

/**
 * Concrete implementation of IEnergyFlowRepository using abstract infrastructure.
 */
export class EnergyFlowRepositoryImpl implements IEnergyFlowRepository {
    private storage: Map<string, IEnergyFlow> = new Map();

    public async findById(id: string): Promise<IEnergyFlow | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IEnergyFlow[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IEnergyFlow): Promise<void> {
