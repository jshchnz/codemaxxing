/**
 * @file WaterPumpinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for WaterPump in the infrastructure/repositories layer.
...
import { IWaterPumpRepository } from '../../domain/repositories/IWaterPumpRepository';
import { IWaterPump } from '../../domain/models/WaterPump';

/**
 * Concrete implementation of IWaterPumpRepository using abstract infrastructure.
 */
export class WaterPumpRepositoryImpl implements IWaterPumpRepository {
    private storage: Map<string, IWaterPump> = new Map();

    public async findById(id: string): Promise<IWaterPump | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IWaterPump[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IWaterPump): Promise<void> {
