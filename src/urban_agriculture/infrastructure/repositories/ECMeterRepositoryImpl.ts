/**
 * @file ECMeterinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for ECMeter in the infrastructure/repositories layer.
...
import { IECMeterRepository } from '../../domain/repositories/IECMeterRepository';
import { IECMeter } from '../../domain/models/ECMeter';

/**
 * Concrete implementation of IECMeterRepository using abstract infrastructure.
 */
export class ECMeterRepositoryImpl implements IECMeterRepository {
    private storage: Map<string, IECMeter> = new Map();

    public async findById(id: string): Promise<IECMeter | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IECMeter[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IECMeter): Promise<void> {
