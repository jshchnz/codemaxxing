/**
 * @file Seedlinginfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Seedling in the infrastructure/repositories layer.
...
import { ISeedlingRepository } from '../../domain/repositories/ISeedlingRepository';
import { ISeedling } from '../../domain/models/Seedling';

/**
 * Concrete implementation of ISeedlingRepository using abstract infrastructure.
 */
export class SeedlingRepositoryImpl implements ISeedlingRepository {
    private storage: Map<string, ISeedling> = new Map();

    public async findById(id: string): Promise<ISeedling | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISeedling[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISeedling): Promise<void> {
