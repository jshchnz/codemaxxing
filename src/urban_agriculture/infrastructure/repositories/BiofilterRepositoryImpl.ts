/**
 * @file Biofilterinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Biofilter in the infrastructure/repositories layer.
...
import { IBiofilterRepository } from '../../domain/repositories/IBiofilterRepository';
import { IBiofilter } from '../../domain/models/Biofilter';

/**
 * Concrete implementation of IBiofilterRepository using abstract infrastructure.
 */
export class BiofilterRepositoryImpl implements IBiofilterRepository {
    private storage: Map<string, IBiofilter> = new Map();

    public async findById(id: string): Promise<IBiofilter | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IBiofilter[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IBiofilter): Promise<void> {
