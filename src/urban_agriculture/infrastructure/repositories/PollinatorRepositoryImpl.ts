/**
 * @file Pollinatorinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Pollinator in the infrastructure/repositories layer.
...
import { IPollinatorRepository } from '../../domain/repositories/IPollinatorRepository';
import { IPollinator } from '../../domain/models/Pollinator';

/**
 * Concrete implementation of IPollinatorRepository using abstract infrastructure.
 */
export class PollinatorRepositoryImpl implements IPollinatorRepository {
    private storage: Map<string, IPollinator> = new Map();

    public async findById(id: string): Promise<IPollinator | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IPollinator[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IPollinator): Promise<void> {
