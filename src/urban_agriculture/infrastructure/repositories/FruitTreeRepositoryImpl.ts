/**
 * @file FruitTreeinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for FruitTree in the infrastructure/repositories layer.
...
import { IFruitTreeRepository } from '../../domain/repositories/IFruitTreeRepository';
import { IFruitTree } from '../../domain/models/FruitTree';

/**
 * Concrete implementation of IFruitTreeRepository using abstract infrastructure.
 */
export class FruitTreeRepositoryImpl implements IFruitTreeRepository {
    private storage: Map<string, IFruitTree> = new Map();

    public async findById(id: string): Promise<IFruitTree | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IFruitTree[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IFruitTree): Promise<void> {
