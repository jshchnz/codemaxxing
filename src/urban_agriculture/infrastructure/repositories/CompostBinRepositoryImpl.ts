/**
 * @file CompostBininfrastructure_repositories.ts
 * @description Enterprise-grade implementation for CompostBin in the infrastructure/repositories layer.
...
import { ICompostBinRepository } from '../../domain/repositories/ICompostBinRepository';
import { ICompostBin } from '../../domain/models/CompostBin';

/**
 * Concrete implementation of ICompostBinRepository using abstract infrastructure.
 */
export class CompostBinRepositoryImpl implements ICompostBinRepository {
    private storage: Map<string, ICompostBin> = new Map();

    public async findById(id: string): Promise<ICompostBin | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ICompostBin[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ICompostBin): Promise<void> {
