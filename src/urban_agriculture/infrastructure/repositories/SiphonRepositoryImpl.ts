/**
 * @file Siphoninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Siphon in the infrastructure/repositories layer.
...
import { ISiphonRepository } from '../../domain/repositories/ISiphonRepository';
import { ISiphon } from '../../domain/models/Siphon';

/**
 * Concrete implementation of ISiphonRepository using abstract infrastructure.
 */
export class SiphonRepositoryImpl implements ISiphonRepository {
    private storage: Map<string, ISiphon> = new Map();

    public async findById(id: string): Promise<ISiphon | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISiphon[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISiphon): Promise<void> {
