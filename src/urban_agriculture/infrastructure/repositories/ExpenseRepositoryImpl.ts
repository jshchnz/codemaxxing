/**
 * @file Expenseinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Expense in the infrastructure/repositories layer.
...
/**
 * Concrete implementation of IExpenseRepository using abstract infrastructure.
 */
export class ExpenseRepositoryImpl implements IExpenseRepository {
    private storage: Map<string, IExpense> = new Map();

    public async findById(id: string): Promise<IExpense | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IExpense[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IExpense): Promise<void> {
