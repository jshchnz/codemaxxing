/**
 * @file Transplantinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Transplant in the infrastructure/repositories layer.
...
import { ITransplantRepository } from '../../domain/repositories/ITransplantRepository';
import { ITransplant } from '../../domain/models/Transplant';

/**
 * Concrete implementation of ITransplantRepository using abstract infrastructure.
 */
export class TransplantRepositoryImpl implements ITransplantRepository {
    private storage: Map<string, ITransplant> = new Map();

    public async findById(id: string): Promise<ITransplant | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ITransplant[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ITransplant): Promise<void> {
