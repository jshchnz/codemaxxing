/**
 * @file Vegetableinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Vegetable in the infrastructure/repositories layer.
...
import { IVegetable } from '../../domain/models/Vegetable';

/**
 * Concrete implementation of IVegetableRepository using abstract infrastructure.
 */
export class VegetableRepositoryImpl implements IVegetableRepository {
    private storage: Map<string, IVegetable> = new Map();

    public async findById(id: string): Promise<IVegetable | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IVegetable[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IVegetable): Promise<void> {
