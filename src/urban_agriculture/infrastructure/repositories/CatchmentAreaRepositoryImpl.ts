/**
 * @file CatchmentAreainfrastructure_repositories.ts
 * @description Enterprise-grade implementation for CatchmentArea in the infrastructure/repositories layer.
...
import { ICatchmentAreaRepository } from '../../domain/repositories/ICatchmentAreaRepository';
import { ICatchmentArea } from '../../domain/models/CatchmentArea';

/**
 * Concrete implementation of ICatchmentAreaRepository using abstract infrastructure.
 */
export class CatchmentAreaRepositoryImpl implements ICatchmentAreaRepository {
    private storage: Map<string, ICatchmentArea> = new Map();

    public async findById(id: string): Promise<ICatchmentArea | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ICatchmentArea[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ICatchmentArea): Promise<void> {
