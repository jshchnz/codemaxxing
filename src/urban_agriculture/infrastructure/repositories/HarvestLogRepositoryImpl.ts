/**
 * @file HarvestLoginfrastructure_repositories.ts
 * @description Enterprise-grade implementation for HarvestLog in the infrastructure/repositories layer.
...
import { IHarvestLogRepository } from '../../domain/repositories/IHarvestLogRepository';
import { IHarvestLog } from '../../domain/models/HarvestLog';

/**
 * Concrete implementation of IHarvestLogRepository using abstract infrastructure.
 */
export class HarvestLogRepositoryImpl implements IHarvestLogRepository {
    private storage: Map<string, IHarvestLog> = new Map();

    public async findById(id: string): Promise<IHarvestLog | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IHarvestLog[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IHarvestLog): Promise<void> {
