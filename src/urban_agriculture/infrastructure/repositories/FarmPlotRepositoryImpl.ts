/**
 * @file FarmPlotinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for FarmPlot in the infrastructure/repositories layer.
...
import { IFarmPlotRepository } from '../../domain/repositories/IFarmPlotRepository';
import { IFarmPlot } from '../../domain/models/FarmPlot';

/**
 * Concrete implementation of IFarmPlotRepository using abstract infrastructure.
 */
export class FarmPlotRepositoryImpl implements IFarmPlotRepository {
    private storage: Map<string, IFarmPlot> = new Map();

    public async findById(id: string): Promise<IFarmPlot | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IFarmPlot[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IFarmPlot): Promise<void> {
