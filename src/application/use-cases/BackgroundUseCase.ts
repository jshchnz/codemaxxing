/**
 * @file Backgroundapplication_use-cases.ts
 * @description Enterprise-grade implementation for Background in the application/use-cases layer.
...
import { IBackground, Background } from '../../domain/models/Background';

/**
 * Enterprise Use Case for managing Background operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageBackgroundUseCase {
    private repository: IBackgroundRepository;

    constructor(repository: IBackgroundRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IBackground> {
        const newEntity = new Background(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IBackground | null> {
