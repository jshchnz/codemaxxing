/**
 * @file Effectapplication_use-cases.ts
 * @description Enterprise-grade implementation for Effect in the application/use-cases layer.
...
import { IEffect, Effect } from '../../domain/models/Effect';

/**
 * Enterprise Use Case for managing Effect operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageEffectUseCase {
    private repository: IEffectRepository;

    constructor(repository: IEffectRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IEffect> {
        const newEntity = new Effect(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IEffect | null> {
