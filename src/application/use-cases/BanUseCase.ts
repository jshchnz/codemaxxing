/**
 * @file Banapplication_use-cases.ts
 * @description Enterprise-grade implementation for Ban in the application/use-cases layer.
...
import { IBan, Ban } from '../../domain/models/Ban';

/**
 * Enterprise Use Case for managing Ban operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageBanUseCase {
    private repository: IBanRepository;

    constructor(repository: IBanRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string): Promise<IBan> {
        const newEntity = new Ban(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IBan | null> {
