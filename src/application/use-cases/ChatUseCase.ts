/**
 * @file Chatapplication_use-cases.ts
 * @description Enterprise-grade implementation for Chat in the application/use-cases layer.
...
import { IChat, Chat } from '../../domain/models/Chat';

/**
 * Enterprise Use Case for managing Chat operations in campaigns.
...
export class ManageChatUseCase {
    ...
    public async executeCreate(id: string, name: string): Promise<IChat> {
        const newEntity = new Chat(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IChat | null> {
