/**
 * @file Encounterapplication_use-cases.ts
 * @description Enterprise-grade implementation for Encounter in the application/use-cases layer.
...
import { IEncounter, Encounter } from '../../domain/models/Encounter';

/**
 * Enterprise Use Case for managing Encounter operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageEncounterUseCase {
    private repository: IEncounterRepository;

    constructor(repository: IEncounterRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IEncounter> {
        const newEntity = new Encounter(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IEncounter | null> {
