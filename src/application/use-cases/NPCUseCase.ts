/**
 * @file NPCapplication_use-cases.ts
 * @description Enterprise-grade implementation for NPC in the application/use-cases layer.
...
import { INPC, NPC } from '../../domain/models/NPC';

/**
 * Enterprise Use Case for managing NPC operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageNPCUseCase {
    private repository: INPCRepository;

    constructor(repository: INPCRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<INPC> {
        const newEntity = new NPC(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<INPC | null> {
