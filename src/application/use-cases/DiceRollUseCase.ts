/**
 * @file DiceRollapplication_use-cases.ts
 * @description Enterprise-grade implementation for DiceRoll in the application/use-cases layer.
...
import { IDiceRoll, DiceRoll } from '../../domain/models/DiceRoll';

/**
 * Enterprise Use Case for managing DiceRoll operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageDiceRollUseCase {
    private repository: IDiceRollRepository;

    constructor(repository: IDiceRollRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IDiceRoll> {
        const newEntity = new DiceRoll(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IDiceRoll | null> {
