/**
 * @file Raceapplication_use-cases.ts
 * @description Enterprise-grade implementation for Race in the application/use-cases layer.
...
import { IRace, Race } from '../../domain/models/Race';

/**
 * Enterprise Use Case for managing Race operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageRaceUseCase {
    ...
    public async executeCreate(id: string, name: string): Promise<IRace> {
        const newEntity = new Race(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IRace | null> {
