/**
 * @file Spellapplication_use-cases.ts
 * @description Enterprise-grade implementation for Spell in the application/use-cases layer.
...
import { ISpell, Spell } from '../../domain/models/Spell';

/**
 * Enterprise Use Case for managing Spell operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageSpellUseCase {
    private repository: ISpellRepository;

    constructor(repository: ISpellRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<ISpell> {
        const newEntity = new Spell(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ISpell | null> {
