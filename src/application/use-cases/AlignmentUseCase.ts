/**
 * @file Alignmentapplication_use-cases.ts
 * @description Enterprise-grade implementation for Alignment in the application/use-cases layer.
...
import { IAlignment, Alignment } from '../../domain/models/Alignment';

/**
 * Enterprise Use Case for managing Alignment operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageAlignmentUseCase {
    private repository: IAlignmentRepository;

    constructor(repository: IAlignmentRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IAlignment> {
        const newEntity = new Alignment(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IAlignment | null> {
