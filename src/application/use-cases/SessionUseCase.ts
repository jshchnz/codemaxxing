/**
 * @file Sessionapplication_use-cases.ts
 * @description Enterprise-grade implementation for Session in the application/use-cases layer.
...
import { ISession, Session } from '../../domain/models/Session';

/**
 * Enterprise Use Case for managing Session operations in campaigns.
...
export class ManageSessionUseCase {
    ...
    public async executeCreate(id: string, name: string): Promise<ISession> {
        const newEntity = new Session(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ISession | null> {
