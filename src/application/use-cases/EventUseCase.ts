/**
 * @file Eventapplication_use-cases.ts
 * @description Enterprise-grade implementation for Event in the application/use-cases layer.
...
import { IEvent, Event } from '../../domain/models/Event';

/**
 * Enterprise Use Case for managing Event operations.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageEventUseCase {
    ...
    public async executeCreate(id: string): Promise<IEvent> {
        const newEntity = new Event(id);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IEvent | null> {
