/**
 * @file Trapapplication_use-cases.ts
 * @description Enterprise-grade implementation for Trap in the application/use-cases layer.
...
import { ITrap, Trap } from '../../domain/models/Trap';

/**
 * Enterprise Use Case for managing Trap operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageTrapUseCase {
    private repository: ITrapRepository;

    constructor(repository: ITrapRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<ITrap> {
        const newEntity = new Trap(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ITrap | null> {
