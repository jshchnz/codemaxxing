/**
 * @file Partyapplication_use-cases.ts
 * @description Enterprise-grade implementation for Party in the application/use-cases layer.
...
import { IParty, Party } from '../../domain/models/Party';

/**
 * Enterprise Use Case for managing Party operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManagePartyUseCase {
    private repository: IPartyRepository;

    constructor(repository: IPartyRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IParty> {
        const newEntity = new Party(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IParty | null> {
