/**
 * @file Lootapplication_use-cases.ts
 * @description Enterprise-grade implementation for Loot in the application/use-cases layer.
...
import { ILoot, Loot } from '../../domain/models/Loot';

/**
 * Enterprise Use Case for managing Loot operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageLootUseCase {
    private repository: ILootRepository;

    constructor(repository: ILootRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<ILoot> {
        const newEntity = new Loot(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ILoot | null> {
