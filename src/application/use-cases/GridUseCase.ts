/**
 * @file Gridapplication_use-cases.ts
 * @description Enterprise-grade implementation for Grid in the application/use-cases layer.
...
import { IGrid, Grid } from '../../domain/models/Grid';

/**
 * Enterprise Use Case for managing Grid operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageGridUseCase {
    private repository: IGridRepository;

    constructor(repository: IGridRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IGrid> {
        const newEntity = new Grid(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IGrid | null> {
