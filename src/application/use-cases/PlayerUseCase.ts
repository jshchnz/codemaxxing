/**
 * @file Playerapplication_use-cases.ts
 * @description Enterprise-grade implementation for Player in the application/use-cases layer.
...
import { IPlayer, Player } from '../../domain/models/Player';

/**
 * Enterprise Use Case for managing Player operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManagePlayerUseCase {
    ...
    public async executeCreate(id: string, name: string): Promise<IPlayer> {
        const newEntity = new Player(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IPlayer | null> {
