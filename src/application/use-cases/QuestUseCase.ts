/**
 * @file Questapplication_use-cases.ts
 * @description Enterprise-grade implementation for Quest in the application/use-cases layer.
...
import { IQuest, Quest } from '../../domain/models/Quest';

/**
 * Enterprise Use Case for managing Quest operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageQuestUseCase {
    private repository: IQuestRepository;

    constructor(repository: IQuestRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IQuest> {
        const newEntity = new Quest(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IQuest | null> {
