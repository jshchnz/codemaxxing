/**
 * @file Mapapplication_use-cases.ts
 * @description Enterprise-grade implementation for Map in the application/use-cases layer.
...
import { IMap, Map } from '../../domain/models/Map';

/**
 * Enterprise Use Case for managing Map operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageMapUseCase {
    private repository: IMapRepository;

    constructor(repository: IMapRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IMap> {
        const newEntity = new Map(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IMap | null> {
