/**
 * @file Itemapplication_use-cases.ts
 * @description Enterprise-grade implementation for Item in the application/use-cases layer.
...
import { IItem, Item } from '../../domain/models/Item';

/**
 * Enterprise Use Case for managing Item operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageItemUseCase {
    ...
    public async executeCreate(id: string, name: string): Promise<IItem> {
        const newEntity = new Item(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IItem | null> {
