/**
 * @file Puzzleapplication_use-cases.ts
 * @description Enterprise-grade implementation for Puzzle in the application/use-cases layer.
...
import { IPuzzle, Puzzle } from '../../domain/models/Puzzle';

/**
 * Enterprise Use Case for managing Puzzle operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManagePuzzleUseCase {
    private repository: IPuzzleRepository;

    constructor(repository: IPuzzleRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IPuzzle> {
        const newEntity = new Puzzle(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IPuzzle | null> {
