/**
 * @file Inventoryapplication_use-cases.ts
 * @description Enterprise-grade implementation for Inventory in the application/use-cases layer.
...
import { IInventory, Inventory } from '../../domain/models/Inventory';

/**
 * Enterprise Use Case for managing Inventory operations in campaigns.
...
export class ManageInventoryUseCase {
    ...
    public async executeCreate(id: string, name: string): Promise<IInventory> {
        const newEntity = new Inventory(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IInventory | null> {
