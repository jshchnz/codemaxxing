/**
 * @file RuleBookapplication_use-cases.ts
 * @description Enterprise-grade implementation for RuleBook in the application/use-cases layer.
...
import { IRuleBook, RuleBook } from '../../domain/models/RuleBook';

/**
 * Enterprise Use Case for managing RuleBook operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageRuleBookUseCase {
    private repository: IRuleBookRepository;

    constructor(repository: IRuleBookRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IRuleBook> {
        const newEntity = new RuleBook(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IRuleBook | null> {
