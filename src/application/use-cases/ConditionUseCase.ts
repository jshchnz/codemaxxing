/**
 * @file Conditionapplication_use-cases.ts
 * @description Enterprise-grade implementation for Condition in the application/use-cases layer.
...
import { ICondition, Condition } from '../../domain/models/Condition';

/**
 * Enterprise Use Case for managing Condition operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageConditionUseCase {
    private repository: IConditionRepository;

    constructor(repository: IConditionRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<ICondition> {
        const newEntity = new Condition(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<ICondition | null> {
