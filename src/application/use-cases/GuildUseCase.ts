/**
 * @file Guildapplication_use-cases.ts
 * @description Enterprise-grade implementation for Guild in the application/use-cases layer.
...
import { IGuild, Guild } from '../../domain/models/Guild';

/**
 * Enterprise Use Case for managing Guild operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageGuildUseCase {
    private repository: IGuildRepository;

    constructor(repository: IGuildRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IGuild> {
        const newEntity = new Guild(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IGuild | null> {
