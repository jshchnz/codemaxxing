/**
 * @file Featapplication_use-cases.ts
 * @description Enterprise-grade implementation for Feat in the application/use-cases layer.
...
import { IFeat, Feat } from '../../domain/models/Feat';

/**
 * Enterprise Use Case for managing Feat operations in campaigns.
 * Orchestrates business rules independently of frameworks.
 */
export class ManageFeatUseCase {
    private repository: IFeatRepository;

    constructor(repository: IFeatRepository) {
        this.repository = repository;
    }

    public async executeCreate(id: string, name: string): Promise<IFeat> {
        const newEntity = new Feat(id, name);
        await this.repository.save(newEntity);
        return newEntity;
    }

    public async executeFind(id: string): Promise<IFeat | null> {
