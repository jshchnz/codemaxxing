/**
 * @file Trapdomain_repositories.ts
 * @description Enterprise-grade implementation for Trap in the domain/repositories layer.
...
import { ITrap } from '../models/Trap';

/**
 * Repository interface for Trap.
 * Follows the Repository pattern to abstract data access.
 */
export interface ITrapRepository {
    findById(id: string): Promise<ITrap | null>;
    findAll(): Promise<ITrap[]>;
    save(entity: ITrap): Promise<void>;
    delete(id: string): Promise<void>;
    findByName(name: string): Promise<ITrap[]>;
}
