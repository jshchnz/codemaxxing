/**
 * @file Alignmentinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Alignment in the infrastructure/repositories layer.
...
import { IAlignmentRepository } from '../../domain/repositories/IAlignmentRepository';
import { IAlignment } from '../../domain/models/Alignment';

/**
 * Concrete implementation of IAlignmentRepository using abstract infrastructure.
 * Perfect for storing Alignment state during intense TTRPG sessions.
 */
export class AlignmentRepositoryImpl implements IAlignmentRepository {
    private storage: Map<string, IAlignment> = new Map();

    public async findById(id: string): Promise<IAlignment | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IAlignment[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IAlignment): Promise<void> {
    ...
    public async findByName(name: string): Promise<IAlignment[]> {
