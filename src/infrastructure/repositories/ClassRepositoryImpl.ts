/**
 * @file Classinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Class in the infrastructure/repositories layer.
...
import { IClass } from '../../domain/models/Class';

/**
 * Concrete implementation of IClassRepository using abstract infrastructure.
 * Perfect for storing Class state during intense TTRPG sessions.
 */
export class ClassRepositoryImpl implements IClassRepository {
    private storage: Map<string, IClass> = new Map();

    public async findById(id: string): Promise<IClass | null> {
    ...
    public async save(entity: IClass): Promise<void> {
    ...
    public async findByName(name: string): Promise<IClass[]> {
