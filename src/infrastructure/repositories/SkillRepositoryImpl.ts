/**
 * @file Skillinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Skill in the infrastructure/repositories layer.
...
import { ISkill } from '../../domain/models/Skill';

/**
 * Concrete implementation of ISkillRepository using abstract infrastructure.
 * Perfect for storing Skill state during intense TTRPG sessions.
 */
export class SkillRepositoryImpl implements ISkillRepository {
    private storage: Map<string, ISkill> = new Map();

    public async findById(id: string): Promise<ISkill | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISkill[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISkill): Promise<void> {
    ...
    public async findByName(name: string): Promise<ISkill[]> {
