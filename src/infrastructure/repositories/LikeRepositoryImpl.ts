/**
 * @file Likeinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Like in the infrastructure/repositories layer.
...
import { ILike } from '../../domain/models/Like';

/**
 * Concrete implementation of ILikeRepository using abstract infrastructure.
 */
export class LikeRepositoryImpl implements ILikeRepository {
    private storage: Map<string, ILike> = new Map();

    public async findById(id: string): Promise<ILike | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ILike[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ILike): Promise<void> {
