/**
 * @file Questinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Quest in the infrastructure/repositories layer.
...
import { IQuest } from '../../domain/models/Quest';

/**
 * Concrete implementation of IQuestRepository using abstract infrastructure.
 * Perfect for storing Quest state during intense TTRPG sessions.
 */
export class QuestRepositoryImpl implements IQuestRepository {
    private storage: Map<string, IQuest> = new Map();

    public async findById(id: string): Promise<IQuest | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IQuest[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IQuest): Promise<void> {
    ...
    public async findByName(name: string): Promise<IQuest[]> {
