/**
 * @file Encounterinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Encounter in the infrastructure/repositories layer.
...
import { IEncounterRepository } from '../../domain/repositories/IEncounterRepository';
import { IEncounter } from '../../domain/models/Encounter';

/**
 * Concrete implementation of IEncounterRepository using abstract infrastructure.
 * Perfect for storing Encounter state during intense TTRPG sessions.
 */
export class EncounterRepositoryImpl implements IEncounterRepository {
    private storage: Map<string, IEncounter> = new Map();

    public async findById(id: string): Promise<IEncounter | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IEncounter[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IEncounter): Promise<void> {
    ...
    public async findByName(name: string): Promise<IEncounter[]> {
