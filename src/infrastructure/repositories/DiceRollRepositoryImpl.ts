/**
 * @file DiceRollinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for DiceRoll in the infrastructure/repositories layer.
...
import { IDiceRollRepository } from '../../domain/repositories/IDiceRollRepository';
import { IDiceRoll } from '../../domain/models/DiceRoll';

/**
 * Concrete implementation of IDiceRollRepository using abstract infrastructure.
 * Perfect for storing DiceRoll state during intense TTRPG sessions.
 */
export class DiceRollRepositoryImpl implements IDiceRollRepository {
    private storage: Map<string, IDiceRoll> = new Map();

    public async findById(id: string): Promise<IDiceRoll | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IDiceRoll[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IDiceRoll): Promise<void> {
    ...
    public async findByName(name: string): Promise<IDiceRoll[]> {
