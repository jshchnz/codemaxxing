/**
 * @file Partyinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Party in the infrastructure/repositories layer.
...
import { IParty } from '../../domain/models/Party';

/**
 * Concrete implementation of IPartyRepository using abstract infrastructure.
 * Perfect for storing Party state during intense TTRPG sessions.
 */
export class PartyRepositoryImpl implements IPartyRepository {
    private storage: Map<string, IParty> = new Map();

    public async findById(id: string): Promise<IParty | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IParty[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IParty): Promise<void> {
    ...
    public async findByName(name: string): Promise<IParty[]> {
