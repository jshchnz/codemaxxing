/**
 * @file Guildinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Guild in the infrastructure/repositories layer.
...
import { IGuildRepository } from '../../domain/repositories/IGuildRepository';
import { IGuild } from '../../domain/models/Guild';

/**
 * Concrete implementation of IGuildRepository using abstract infrastructure.
 * Perfect for storing Guild state during intense TTRPG sessions.
 */
export class GuildRepositoryImpl implements IGuildRepository {
    private storage: Map<string, IGuild> = new Map();

    public async findById(id: string): Promise<IGuild | null> {
    ...
    public async save(entity: IGuild): Promise<void> {
    ...
    public async findByName(name: string): Promise<IGuild[]> {
