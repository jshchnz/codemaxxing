/**
 * @file Searchinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Search in the infrastructure/repositories layer.
...
import { ISearch } from '../../domain/models/Search';

/**
 * Concrete implementation of ISearchRepository using abstract infrastructure.
 */
export class SearchRepositoryImpl implements ISearchRepository {
    private storage: Map<string, ISearch> = new Map();

    public async findById(id: string): Promise<ISearch | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISearch[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISearch): Promise<void> {
