/**
 * @file Advertisementinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Advertisement in the infrastructure/repositories layer.
...
import { IAdvertisement } from '../../domain/models/Advertisement';

/**
 * Concrete implementation of IAdvertisementRepository using abstract infrastructure.
 */
export class AdvertisementRepositoryImpl implements IAdvertisementRepository {
    private storage: Map<string, IAdvertisement> = new Map();

    public async findById(id: string): Promise<IAdvertisement | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IAdvertisement[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IAdvertisement): Promise<void> {
