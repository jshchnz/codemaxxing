/**
 * @file Inventoryinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Inventory in the infrastructure/repositories layer.
...
import { IInventory } from '../../domain/models/Inventory';

/**
 * Concrete implementation of IInventoryRepository using abstract infrastructure.
 * Perfect for storing Inventory state during intense TTRPG sessions.
 */
export class InventoryRepositoryImpl implements IInventoryRepository {
    private storage: Map<string, IInventory> = new Map();

    public async findById(id: string): Promise<IInventory | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IInventory[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IInventory): Promise<void> {
    ...
    public async findByName(name: string): Promise<IInventory[]> {
