/**
 * @file Labelinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Label in the infrastructure/repositories layer.
...
import { ILabel } from '../../domain/models/Label';

/**
 * Concrete implementation of ILabelRepository using abstract infrastructure.
 */
export class LabelRepositoryImpl implements ILabelRepository {
    private storage: Map<string, ILabel> = new Map();

    public async findById(id: string): Promise<ILabel | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ILabel[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ILabel): Promise<void> {
