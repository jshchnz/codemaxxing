/**
 * @file Chatinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Chat in the infrastructure/repositories layer.
...
/**
 * Concrete implementation of IChatRepository using abstract infrastructure.
 * Perfect for storing Chat state during intense TTRPG sessions.
 */
export class ChatRepositoryImpl implements IChatRepository {
    private storage: Map<string, IChat> = new Map();

    public async findById(id: string): Promise<IChat | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IChat[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IChat): Promise<void> {
    ...
    public async findByName(name: string): Promise<IChat[]> {
