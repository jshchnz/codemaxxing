/**
 * @file Backgroundinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Background in the infrastructure/repositories layer.
...
import { IBackground } from '../../domain/models/Background';

/**
 * Concrete implementation of IBackgroundRepository using abstract infrastructure.
 * Perfect for storing Background state during intense TTRPG sessions.
 */
export class BackgroundRepositoryImpl implements IBackgroundRepository {
    private storage: Map<string, IBackground> = new Map();

    public async findById(id: string): Promise<IBackground | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IBackground[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IBackground): Promise<void> {
    ...
    public async findByName(name: string): Promise<IBackground[]> {
