/**
 * @file Sessioninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Session in the infrastructure/repositories layer.
...
import { ISession } from '../../domain/models/Session';

/**
 * Concrete implementation of ISessionRepository using abstract infrastructure.
 * Perfect for storing Session state during intense TTRPG sessions.
 */
export class SessionRepositoryImpl implements ISessionRepository {
    private storage: Map<string, ISession> = new Map();

    public async findById(id: string): Promise<ISession | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISession[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISession): Promise<void> {
    ...
    public async findByName(name: string): Promise<ISession[]> {
