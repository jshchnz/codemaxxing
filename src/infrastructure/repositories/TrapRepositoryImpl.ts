/**
 * @file Trapinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Trap in the infrastructure/repositories layer.
...
import { ITrapRepository } from '../../domain/repositories/ITrapRepository';
import { ITrap } from '../../domain/models/Trap';

/**
 * Concrete implementation of ITrapRepository using abstract infrastructure.
 * Perfect for storing Trap state during intense TTRPG sessions.
 */
export class TrapRepositoryImpl implements ITrapRepository {
    private storage: Map<string, ITrap> = new Map();

    public async findById(id: string): Promise<ITrap | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ITrap[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ITrap): Promise<void> {
    ...
    public async findByName(name: string): Promise<ITrap[]> {
