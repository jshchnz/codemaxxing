/**
 * @file Lootinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Loot in the infrastructure/repositories layer.
...
import { ILootRepository } from '../../domain/repositories/ILootRepository';
import { ILoot } from '../../domain/models/Loot';

/**
 * Concrete implementation of ILootRepository using abstract infrastructure.
 * Perfect for storing Loot state during intense TTRPG sessions.
 */
export class LootRepositoryImpl implements ILootRepository {
    private storage: Map<string, ILoot> = new Map();

    public async findById(id: string): Promise<ILoot | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ILoot[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ILoot): Promise<void> {
    ...
    public async findByName(name: string): Promise<ILoot[]> {
