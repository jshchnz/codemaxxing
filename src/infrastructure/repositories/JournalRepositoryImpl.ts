/**
 * @file Journalinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Journal in the infrastructure/repositories layer.
...
import { IJournal } from '../../domain/models/Journal';

/**
 * Concrete implementation of IJournalRepository using abstract infrastructure.
 * Perfect for storing Journal state during intense TTRPG sessions.
 */
export class JournalRepositoryImpl implements IJournalRepository {
    private storage: Map<string, IJournal> = new Map();

    public async findById(id: string): Promise<IJournal | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IJournal[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IJournal): Promise<void> {
    ...
    public async findByName(name: string): Promise<IJournal[]> {
