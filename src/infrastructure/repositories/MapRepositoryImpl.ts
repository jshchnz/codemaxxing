/**
 * @file Mapinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Map in the infrastructure/repositories layer.
...
import { IMap } from '../../domain/models/Map';

/**
 * Concrete implementation of IMapRepository using abstract infrastructure.
 * Perfect for storing Map state during intense TTRPG sessions.
 */
export class MapRepositoryImpl implements IMapRepository {
    private storage: Map<string, IMap> = new Map();

    public async findById(id: string): Promise<IMap | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IMap[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IMap): Promise<void> {
    ...
    public async findByName(name: string): Promise<IMap[]> {
