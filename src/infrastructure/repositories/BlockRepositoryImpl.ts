/**
 * @file Blockinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Block in the infrastructure/repositories layer.
...
/**
 * Concrete implementation of IBlockRepository using abstract infrastructure.
 */
export class BlockRepositoryImpl implements IBlockRepository {
    private storage: Map<string, IBlock> = new Map();

    public async findById(id: string): Promise<IBlock | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IBlock[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IBlock): Promise<void> {
