/**
 * @file Featinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Feat in the infrastructure/repositories layer.
...
import { IFeat } from '../../domain/models/Feat';

/**
 * Concrete implementation of IFeatRepository using abstract infrastructure.
 * Perfect for storing Feat state during intense TTRPG sessions.
 */
export class FeatRepositoryImpl implements IFeatRepository {
    private storage: Map<string, IFeat> = new Map();

    public async findById(id: string): Promise<IFeat | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IFeat[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IFeat): Promise<void> {
    ...
    public async findByName(name: string): Promise<IFeat[]> {
