/**
 * @file Spellinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Spell in the infrastructure/repositories layer.
...
import { ISpell } from '../../domain/models/Spell';

/**
 * Concrete implementation of ISpellRepository using abstract infrastructure.
 * Perfect for storing Spell state during intense TTRPG sessions.
 */
export class SpellRepositoryImpl implements ISpellRepository {
    private storage: Map<string, ISpell> = new Map();

    public async findById(id: string): Promise<ISpell | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ISpell[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ISpell): Promise<void> {
    ...
    public async findByName(name: string): Promise<ISpell[]> {
