/**
 * @file Conditioninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Condition in the infrastructure/repositories layer.
...
import { ICondition } from '../../domain/models/Condition';

/**
 * Concrete implementation of IConditionRepository using abstract infrastructure.
 * Perfect for storing Condition state during intense TTRPG sessions.
 */
export class ConditionRepositoryImpl implements IConditionRepository {
    private storage: Map<string, ICondition> = new Map();

    public async findById(id: string): Promise<ICondition | null> {
    ...
    public async save(entity: ICondition): Promise<void> {
    ...
    public async findByName(name: string): Promise<ICondition[]> {
