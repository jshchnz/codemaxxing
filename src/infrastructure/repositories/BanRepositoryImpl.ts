/**
 * @file Baninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Ban in the infrastructure/repositories layer.
...
import { IBanRepository } from '../../domain/repositories/IBanRepository';
import { IBan } from '../../domain/models/Ban';

/**
 * Concrete implementation of IBanRepository using abstract infrastructure.
 */
export class BanRepositoryImpl implements IBanRepository {
    private storage: Map<string, IBan> = new Map();

    public async findById(id: string): Promise<IBan | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IBan[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IBan): Promise<void> {
