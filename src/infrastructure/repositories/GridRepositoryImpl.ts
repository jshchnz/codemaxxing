/**
 * @file Gridinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Grid in the infrastructure/repositories layer.
...
import { IGridRepository } from '../../domain/repositories/IGridRepository';
import { IGrid } from '../../domain/models/Grid';

/**
 * Concrete implementation of IGridRepository using abstract infrastructure.
 * Perfect for storing Grid state during intense TTRPG sessions.
 */
export class GridRepositoryImpl implements IGridRepository {
    private storage: Map<string, IGrid> = new Map();

    public async findById(id: string): Promise<IGrid | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IGrid[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IGrid): Promise<void> {
    ...
    public async findByName(name: string): Promise<IGrid[]> {
