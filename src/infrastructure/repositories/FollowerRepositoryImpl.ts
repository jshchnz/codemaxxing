/**
 * @file Followerinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Follower in the infrastructure/repositories layer.
...
import { IFollowerRepository } from '../../domain/repositories/IFollowerRepository';
import { IFollower } from '../../domain/models/Follower';

/**
 * Concrete implementation of IFollowerRepository using abstract infrastructure.
 */
export class FollowerRepositoryImpl implements IFollowerRepository {
    private storage: Map<string, IFollower> = new Map();

    public async findById(id: string): Promise<IFollower | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IFollower[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IFollower): Promise<void> {
