/**
 * @file Analyticsinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Analytics in the infrastructure/repositories layer.
...
import { IAnalytics } from '../../domain/models/Analytics';

/**
 * Concrete implementation of IAnalyticsRepository using abstract infrastructure.
 */
export class AnalyticsRepositoryImpl implements IAnalyticsRepository {
    private storage: Map<string, IAnalytics> = new Map();

    public async findById(id: string): Promise<IAnalytics | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IAnalytics[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IAnalytics): Promise<void> {
