/**
 * @file Streaminfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Stream in the infrastructure/repositories layer.
...
import { IStream } from '../../domain/models/Stream';

/**
 * Concrete implementation of IStreamRepository using abstract infrastructure.
 */
export class StreamRepositoryImpl implements IStreamRepository {
    private storage: Map<string, IStream> = new Map();

    public async findById(id: string): Promise<IStream | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IStream[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IStream): Promise<void> {
