/**
 * @file Royaltyinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Royalty in the infrastructure/repositories layer.
...
import { IRoyaltyRepository } from '../../domain/repositories/IRoyaltyRepository';
import { IRoyalty } from '../../domain/models/Royalty';

/**
 * Concrete implementation of IRoyaltyRepository using abstract infrastructure.
 */
export class RoyaltyRepositoryImpl implements IRoyaltyRepository {
    private storage: Map<string, IRoyalty> = new Map();

    public async findById(id: string): Promise<IRoyalty | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IRoyalty[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IRoyalty): Promise<void> {
