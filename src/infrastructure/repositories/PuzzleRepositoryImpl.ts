/**
 * @file Puzzleinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Puzzle in the infrastructure/repositories layer.
...
import { IPuzzleRepository } from '../../domain/repositories/IPuzzleRepository';
import { IPuzzle } from '../../domain/models/Puzzle';

/**
 * Concrete implementation of IPuzzleRepository using abstract infrastructure.
 * Perfect for storing Puzzle state during intense TTRPG sessions.
 */
export class PuzzleRepositoryImpl implements IPuzzleRepository {
    private storage: Map<string, IPuzzle> = new Map();

    public async findById(id: string): Promise<IPuzzle | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IPuzzle[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IPuzzle): Promise<void> {
    ...
    public async findByName(name: string): Promise<IPuzzle[]> {
