/**
 * @file RuleBookinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for RuleBook in the infrastructure/repositories layer.
...
import { IRuleBookRepository } from '../../domain/repositories/IRuleBookRepository';
import { IRuleBook } from '../../domain/models/RuleBook';

/**
 * Concrete implementation of IRuleBookRepository using abstract infrastructure.
 * Perfect for storing RuleBook state during intense TTRPG sessions.
 */
export class RuleBookRepositoryImpl implements IRuleBookRepository {
    private storage: Map<string, IRuleBook> = new Map();

    public async findById(id: string): Promise<IRuleBook | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IRuleBook[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IRuleBook): Promise<void> {
    ...
    public async findByName(name: string): Promise<IRuleBook[]> {
