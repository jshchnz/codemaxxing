/**
 * @file Bestiaryinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Bestiary in the infrastructure/repositories layer.
...
import { IBestiary } from '../../domain/models/Bestiary';

/**
 * Concrete implementation of IBestiaryRepository using abstract infrastructure.
 * Perfect for storing Bestiary state during intense TTRPG sessions.
 */
export class BestiaryRepositoryImpl implements IBestiaryRepository {
    private storage: Map<string, IBestiary> = new Map();

    public async findById(id: string): Promise<IBestiary | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IBestiary[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IBestiary): Promise<void> {
    ...
    public async findByName(name: string): Promise<IBestiary[]> {
