/**
 * @file Raceinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Race in the infrastructure/repositories layer.
...
import { IRace } from '../../domain/models/Race';

/**
 * Concrete implementation of IRaceRepository using abstract infrastructure.
 * Perfect for storing Race state during intense TTRPG sessions.
 */
export class RaceRepositoryImpl implements IRaceRepository {
    private storage: Map<string, IRace> = new Map();

    public async findById(id: string): Promise<IRace | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<IRace[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: IRace): Promise<void> {
    ...
    public async findByName(name: string): Promise<IRace[]> {
