/**
 * @file Trackinfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Track in the infrastructure/repositories layer.
...
import { ITrack } from '../../domain/models/Track';

/**
 * Concrete implementation of ITrackRepository using abstract infrastructure.
 */
export class TrackRepositoryImpl implements ITrackRepository {
    private storage: Map<string, ITrack> = new Map();

    public async findById(id: string): Promise<ITrack | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ITrack[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ITrack): Promise<void> {
