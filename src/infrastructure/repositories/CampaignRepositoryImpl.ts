/**
 * @file Campaigninfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Campaign in the infrastructure/repositories layer.
...
import { ICampaign } from '../../domain/models/Campaign';

/**
 * Concrete implementation of ICampaignRepository using abstract infrastructure.
 * Perfect for storing Campaign state during intense TTRPG sessions.
 */
export class CampaignRepositoryImpl implements ICampaignRepository {
    private storage: Map<string, ICampaign> = new Map();

    public async findById(id: string): Promise<ICampaign | null> {
        return this.storage.get(id) || null;
    }

    public async findAll(): Promise<ICampaign[]> {
        return Array.from(this.storage.values());
    }

    public async save(entity: ICampaign): Promise<void> {
    ...
    public async findByName(name: string): Promise<ICampaign[]> {
