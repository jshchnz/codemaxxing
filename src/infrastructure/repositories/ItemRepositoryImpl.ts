/**
 * @file Iteminfrastructure_repositories.ts
 * @description Enterprise-grade implementation for Item in the infrastructure/repositories layer.
...
import { IItem } from '../../domain/models/Item';

/**
 * Concrete implementation of IItemRepository using abstract infrastructure.
 * Perfect for storing Item state during intense TTRPG sessions.
 */
export class ItemRepositoryImpl implements IItemRepository {
    private storage: Map<string, IItem> = new Map();
    ...
    public async save(entity: IItem): Promise<void> {
    ...
    public async findByName(name: string): Promise<IItem[]> {
